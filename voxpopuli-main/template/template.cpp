...
#else
	renderTarget = new GLTexture(scrwidth, scrheight, GLTexture::DEFAULT);
#endif
}
...
void KeyEventCallback(GLFWwindow*, int key, int, int action, int) {
	...
	if (action == GLFW_PRESS) { if (app) if (key >= 0) app->KeyDown(key); keystate[key & 255] = 1; } else if (action == GLFW_RELEASE) { if (app) if (key >= 0) app->KeyUp(key); keystate[key & 255] = 0; }
}
...
void MouseButtonCallback(GLFWwindow*, int button, int action, int) {
	if (action == GLFW_PRESS) { if (app) app->MouseDown(button); } else if (action == GLFW_RELEASE) { if (app) app->MouseUp(button); }
}
...
#if HDR
	FLoatSurface* screen = new FLoatSurface(SCRWIDTH, SCRHEIGHT);
#else
